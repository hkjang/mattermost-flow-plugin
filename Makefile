...
ifneq ($(HAS_WEBAPP),)
	...
	./build/bin/manifest bundle $(PLUGIN_ID) $(BUNDLE_NAME)
...
ifneq ($(HAS_SERVER),)
	...
	mockgen -destination=server/command/mocks/mock_commands.go -package=mocks github.com/hkjang/mattermost-flow-plugin/server/command Command
endif
